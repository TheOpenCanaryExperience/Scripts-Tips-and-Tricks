...
def extract_content(log_file, system_name, days_back):
    filtered_folder = os.path.join(os.environ['HOME'], 'logs', 'reporting', system_name, 'filtered')
    os.makedirs(filtered_folder, exist_ok=True)

    date_days_back = (datetime.datetime.now() - datetime.timedelta(days=int(days_back))).strftime('%Y%m%d')
    port_filename = f'port_{system_name}_{date_days_back}.log'
    username_filename = f'username_{system_name}_{date_days_back}.log'
    password_filename = f'password_{system_name}_{date_days_back}.log'
    vncpass_filename = f'vncpass_{system_name}_{date_days_back}.log'
    src_host_filename = f'src_host_{system_name}_{date_days_back}.log'
...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        print("Usage: ./log_extractor.py <system_name> <days_back>")
        sys.exit(1)
    ...
    # Get specified number of days back
    date_days_back = (datetime.datetime.now() - datetime.timedelta(days=int(days_back))).strftime('%Y%m%d')

    # Build the log filename for the specified number of days back
    log_filename = f'opencanary.log_{date_days_back}'

    # Full path to the log file
    log_file_path = os.path.join(log_folder, log_filename)

    # Call the function to extract content
    extract_content(log_file_path, system_name, days_back)
